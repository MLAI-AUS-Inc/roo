...
**Successful reports are visible to everyone in the conversation.** In a channel,
Roo posts the report at channel level. In an existing thread, it replies in the
same thread. Only admins can request a report; errors and denials stay private. Roo must have access to the conversation; this is not a command
...
for DMs), with `chat:write` for replies. **No new slash command needs
...
finishes. Successful delivery completes the receipt; failed delivery releases
it for retry. Retries preserve visibility and never fall back to AI. Successful
reports use ordinary Slack messages; errors and denials use ephemeral replies.
...
- Admin channel and thread requests get active-booking lists visible to conversation members.
...
- Backend errors and authorization denials never publish booking names.
